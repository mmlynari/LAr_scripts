...
f_sig = "/eos/user/b/brfranco/EMDesignFiles/cross_talk_results/2_shields_gnd_shortened_voltages.csv"
...
plotfolder = "xtalk_plots_ANSYS_prototype_v0_cell7input_2shields_AirGap1do8mm_voltages"
...
for header in headers:
    if x_axis_string_from_csv in header:
        ...
        continue
    df_sig = df_sig.rename(columns={header:header.replace(" [V]", "").replace("V(", "").replace(")", "").replace("Inegative(", "").replace("_O", "").replace('cell', 'Cell ')})
...
plt.clf()
df_sig.plot(x = x_axis_string_from_csv, ylabel = "Voltage [V]", subplots = True, xlim = (0, my_timeLimit), layout = (int(n_columns/2.0) + 1, 2), figsize = (12, 12), grid = True)
...
for tau in taus:
    ...
    for label, currents in df_sig.items():
        if label == x_axis_string_from_csv or "Input" in label:
            continue
