...
data_file_pattern = "*input-tower02cell07*.txt"
#output_plot_folder = os.path.join(input_data_folder, "plots_trialWithStrips")
#output_plot_folder = os.path.join(input_data_folder, "plots_1MOhmInjector_complete_zeroShields")
output_plot_folder = os.path.join(input_data_folder, "plots_50OhmInjector_complete_twoShields")
if not os.path.isdir(output_plot_folder):
    os.mkdir(output_plot_folder)

# Retrieve tables from simulation for comparisons
simu_0_shield_table = """
No shaper & 2.58 & 0.64 & 0.67 & 3.58 & 3.37 & 8.72  \\
20 & 0.67 & 0.09 & 0.08 & 0.95 & 0.88 & 2.48  \\
50 & 0.31 & 0.02 & 0.02 & 0.43 & 0.4 & 1.14  \\
100 & 0.17 & 0.01 & 0.01 & 0.24 & 0.23 & 0.65  \\
150 & 0.13 & 0.01 & 0.0 & 0.18 & 0.17 & 0.49  \\
200 & 0.11 & 0.01 & 0.0 & 0.15 & 0.14 & 0.41  \\
300 & 0.08 & 0.0 & 0.0 & 0.12 & 0.11 & 0.32  \\
""".split("\\")
simu_1_shield_table = """
No shaper & 1.24 & 0.25 & 0.23 & 1.79 & 1.8 & 4.66  \\
20 & 0.39 & 0.05 & 0.04 & 0.55 & 0.55 & 1.69  \\
50 & 0.18 & 0.02 & 0.01 & 0.26 & 0.25 & 0.81  \\
100 & 0.1 & 0.01 & 0.0 & 0.15 & 0.14 & 0.47  \\
150 & 0.08 & 0.01 & 0.0 & 0.11 & 0.11 & 0.35  \\
200 & 0.06 & 0.01 & 0.0 & 0.09 & 0.09 & 0.29  \\
300 & 0.05 & 0.0 & 0.0 & 0.07 & 0.07 & 0.24  \\
""".split("\\")
simu_2_shield_table = """
No shaper & 0.13 & 0.12 & 0.07 & 0.28 & 0.33 & 2.29  \\
20 & 0.03 & 0.03 & 0.0 & 0.08 & 0.1 & 0.79  \\
50 & 0.01 & 0.01 & 0.0 & 0.04 & 0.05 & 0.4  \\
100 & 0.01 & 0.01 & 0.0 & 0.02 & 0.03 & 0.25  \\
150 & 0.01 & 0.01 & 0.0 & 0.02 & 0.02 & 0.19  \\
200 & 0.0 & 0.0 & 0.0 & 0.01 & 0.02 & 0.17  \\
300 & 0.0 & 0.0 & 0.0 & 0.01 & 0.01 & 0.14  \\
""".split("\\")
...
input_tower_type = ""
simu_to_compare_to = simu_2_shield_table
if ref_tower == str(2):
    input_tower_type = " baseline "
if ref_tower == str(8):
    input_tower_type = " no shield "
    simu_to_compare_to = simu_0_shield_table
if ref_tower == str(7):
    input_tower_type = " one shield "
    simu_to_compare_to = simu_1_shield_table
if ref_tower == str(4):
...
simu_to_compare_to = [item.split(" & ") for item in simu_to_compare_to]
print("Will compare to table:\n", simu_to_compare_to)
...
template_string_xtalk_comparison_table = """
\\begin{table}[h!]
\centering
\\begin{tabular}{c|COLUMN}
\stackunder{Cross-talk meas/simu}{Shaping time (ns) $\downarrow$} & CELLS \\\\
\hline
BODY
\end{tabular}
\caption{Ratio between measured and simulated cross-talk (injection on cell NUM), \\textbf{TOWERTYPE tower}.}
\label{xtalk}
\end{table}
""".replace("COLUMN", column_string).replace('CELLS', cell_header).replace('TOWERTYPE', input_tower_type).replace("NUM", ref_cell)

body_string = ""
body_string_comparison = ""
shaping_time_index_for_comparison = 0
for tau in taus:
    values_to_compare_to = simu_to_compare_to[shaping_time_index_for_comparison]
    if tau == 0:
        body_string += "No shaper &"
        body_string_comparison += "No shaper &"
    else:
        body_string += str(tau) + " &"
        body_string_comparison += str(tau) + " &"
    cell_idx_for_comparison = 0
    for cell in sorted(cells):
        current_tower = cell.split("-")[0]
        current_cell = cell.split("-")[1]
        if current_tower == ref_tower and current_cell == ref_cell:
            continue
        simulated_cross_talk = float(values_to_compare_to[cell_idx_for_comparison + 1])
        ...
        x_talk_ratio = round(x_talk / simulated_cross_talk, 2)
        body_string_comparison += " " + str(x_talk_ratio) + " &"
        cell_idx_for_comparison += 1
    body_string = body_string[:-1] + " \\\\\n"
    body_string_comparison = body_string_comparison[:-1] + " \\\\\n"
    shaping_time_index_for_comparison += 1
body_string = body_string[:-1]
body_string_comparison = body_string_comparison[:-1]
template_string_xtalk_table = template_string_xtalk_table.replace('BODY', body_string)
template_string_xtalk_comparison_table = template_string_xtalk_comparison_table.replace('BODY', body_string_comparison)

with open(os.path.join(output_plot_folder, "table.txt"), "a") as texFile:
    texFile.write(template_string_xtalk_table)
with open(os.path.join(output_plot_folder, "comparison_table.txt"), "a") as texFile:
    texFile.write(template_string_xtalk_comparison_table)
print(template_string_xtalk_table)
print(os.path.join(output_plot_folder, "table.txt"), " written.")
print(os.path.join(output_plot_folder, "comparison_table.txt"), " written.")
print(template_string_xtalk_comparison_table)
