...
if not os.path.isfile(rootfile_path):
    ...
    sys.exit(1)
plot_dir_name = 'plots_averaged_showerShape_' + date.today().strftime("%y%m%d")  #+ "_" + os.path.basename(rootfile_path).replace('.root','')
...
tprof_energy_vs_depth = ROOT.TProfile("tprof_energy_vs_depth_" + energy_str_gev, "tprof_energy_vs_depth_" + energy_str_gev, 100, 2100, 2600)
...
for event in events:
    ...
    for cell_idx in xrange(len(event.ECalBarrelPositionedCells_energy)):
        ...
        tprof_energy_vs_depth.Fill(sqrt(event.ECalBarrelPositionedCells_x[cell_idx] * event.ECalBarrelPositionedCells_x[cell_idx] + event.ECalBarrelPositionedCells_y[cell_idx] * event.ECalBarrelPositionedCells_y[cell_idx]), 1000*100*event.ECalBarrelPositionedCells_energy[cell_idx]/(float(energy)))
...
tprof_energy_vs_depth.SetLineWidth(2)
tprof_energy_vs_depth.GetXaxis().SetTitle("Radial depth (mm)")
tprof_energy_vs_depth.GetYaxis().SetTitle("Average nergy deposit (a.u.)")
