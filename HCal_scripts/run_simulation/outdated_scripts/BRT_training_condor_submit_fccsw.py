...
if __name__ == "__main__":
    ...
    parser.add_argument("-outputFolder", default = "/eos/user/m/mmlynari/FCC_rootfile_storage/", help = "Output folder absolute path for the rootfile", type = str)
    ...
    parser.add_argument("-polarAngles", default = [69.805], help="Polar angles in degrees for the process to generate, behavior depends on fixedPolarAngles", type = int, nargs = '+')
    ...
    parser.add_argument("-nEvt", default = 300000, help = "How many events to generate.", type = int)
    ...
    fcc_analysis_path = "/afs/cern.ch/user/m/mmlynari/workspace/FCC_MR_benchmark_16Apr24/FCCAnalyses"
        ...
        for theta in thetas_for_loop:
            ...
            if args.jobType == 'samplingFraction':
                ...
                if args.pythia:
                    ...
                    fcc_analysis_commands.append("python examples/FCCee/fullSim/caloNtupleizer/analysis_HCal.py -inputFiles OUTPUTDIR/fccsw_output_pdgID_PDGID_pMin_PMIN_pMax_PMAX_thetaMin_THETAMIN_thetaMax_THETAMAX.root -outputFolder FCCANAOUTPUT_caloReco\n".replace('PMIN', str(energy_min)).replace('PMAX', str(energy_max)).replace('OUTPUTDIR', outfile_storage).replace('PDGID', str(pdgid)).replace('THETAMIN', str(theta_min)).replace('THETAMAX', str(theta_max)).replace("FCCANAOUTPUT", fcc_ana_output_dir))
