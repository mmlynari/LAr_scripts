...
def main():
    ...
    parser.add_argument("--numLayersHCal", default=13, type=int)
    ...
    run(args.inputDir, args.clusters, args.outFile, args.training, args.writeFeatures, args.writeTarget, args.numLayersHCal, args.useShapeParameters)


def run(in_directory, clusters, out_file, runTraining, writeFeatures, writeTarget, numLayersHCal, useShapeParameters):
    """Actual processing"""
    ...
    if useShapeParameters:
        ...
        df = (
            df
            .Define(f"{clusters}_energyInLayersHCal", f"getCaloCluster_energyInLayersHCal({clusters}, {cells}, {numLayersHCal})")
    ...
    if useShapeParameters:
        ...
        normalized_layers = np.array([cols[f"Cluster_E{i}"] for i in range(numLayersHCal)])
    else:
        layers = np.array([cols[f"Cluster_E{i}"] for i in range(numLayersHCal)])
    ...
    if runTraining:
        ...
        reg.subsample = 0.997
        reg.max_depth = 4
        reg.n_estimators = 526
        reg.learning_rate = 0.239
        # reg.subsample = 0.8
        # reg.max_depth = 4
